%%% % DO NOT EDIT: this file was generated by 'just codegen'
%%% % @generated SignedSource<<faccd0a5f7464c7fe46851477133df53>>
%%%-----------------------------------------------------------------------------
%%% %CopyrightBegin%
%%%
%%% SPDX-License-Identifier: Apache-2.0
%%%
%%% Copyright (c) Meta Platforms, Inc. and affiliates.
%%% Copyright (c) WhatsApp LLC
%%%
%%% Licensed under the Apache License, Version 2.0 (the "License");
%%% you may not use this file except in compliance with the License.
%%% You may obtain a copy of the License at
%%%
%%%     http://www.apache.org/licenses/LICENSE-2.0
%%%
%%% Unless required by applicable law or agreed to in writing, software
%%% distributed under the License is distributed on an "AS IS" BASIS,
%%% WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
%%% See the License for the specific language governing permissions and
%%% limitations under the License.
%%%
%%% %CopyrightEnd%
%%%-----------------------------------------------------------------------------
%%% % @format
-module(markdown_fuzz_SUITE).
-moduledoc """

""".
-moduledoc #{author => ["Andrew Bennett <potatosaladx@meta.com>"]}.
-moduledoc #{created => "2025-10-28", modified => "2025-10-28"}.
-moduledoc #{copyright => "Meta Platforms, Inc. and affiliates."}.
-compile(warn_missing_spec_all).
-oncall("whatsapp_clr").

-include_lib("markdown/include/markdown_mdast.hrl").
-include_lib("markdown/include/markdown_util.hrl").
-include_lib("stdlib/include/assert.hrl").

-behaviour(ct_suite).

%% ct_suite callbacks
-export([
    all/0,
    groups/0,
    init_per_suite/1,
    end_per_suite/1,
    init_per_group/2,
    end_per_group/2
]).

%% Test Cases
-export([
    test_fuzz_case_1/1,
    %% The first link is stopped by the `+` (so it’s `a@b.c`), but the next link overlaps it (`b.c+d@e.f`).
    test_fuzz_case_2/1,
    test_fuzz_case_3/1,
    test_fuzz_case_4/1,
    test_fuzz_case_5/1,
    test_fuzz_case_6/1,
    test_fuzz_case_7/1,
    test_fuzz_case_8/1,
    test_fuzz_case_9/1,
    test_fuzz_case_10/1,
    test_fuzz_case_11/1,
    test_fuzz_case_12/1,
    test_fuzz_case_13/1,
    test_fuzz_case_14/1,
    test_fuzz_case_15/1,
    test_fuzz_case_16/1,
    test_fuzz_case_17/1,
    test_fuzz_case_18/1,
    test_fuzz_case_19/1
]).

%%%=============================================================================
%%% ct_suite callbacks
%%%=============================================================================

-spec all() -> markdown_test:all().
all() ->
    [
        {group, static}
    ].

-spec groups() -> markdown_test:groups().
groups() ->
    [
        {static, [parallel], [
            test_fuzz_case_1,
            %% The first link is stopped by the `+` (so it’s `a@b.c`), but the next link overlaps it (`b.c+d@e.f`).
            test_fuzz_case_2,
            test_fuzz_case_3,
            test_fuzz_case_4,
            test_fuzz_case_5,
            test_fuzz_case_6,
            test_fuzz_case_7,
            test_fuzz_case_8,
            test_fuzz_case_9,
            test_fuzz_case_10,
            test_fuzz_case_11,
            test_fuzz_case_12,
            test_fuzz_case_13,
            test_fuzz_case_14,
            test_fuzz_case_15,
            test_fuzz_case_16,
            test_fuzz_case_17,
            test_fuzz_case_18,
            test_fuzz_case_19
        ]}
    ].

-spec init_per_suite(Config :: ct_suite:ct_config()) -> markdown_test:init_per_suite().
init_per_suite(Config) ->
    Config.

-spec end_per_suite(Config :: ct_suite:ct_config()) -> markdown_test:end_per_suite().
end_per_suite(_Config) ->
    ok.

-spec init_per_group(GroupName :: ct_suite:ct_groupname(), Config :: ct_suite:ct_config()) ->
    markdown_test:init_per_group().
init_per_group(_Group, Config) ->
    Config.

-spec end_per_group(GroupName :: ct_suite:ct_groupname(), Config :: ct_suite:ct_config()) ->
    markdown_test:end_per_group().
end_per_group(_Group, _Config) ->
    ok.

%%%=============================================================================
%%% Test Cases
%%%=============================================================================

-spec test_fuzz_case_1(Config) -> markdown_test:testcase() when
    Config :: ct_suite:ct_config().
test_fuzz_case_1(_Config) ->
    ?assertMatch(
        {ok, <<"<h2>[\n~\na</h2>\n"/utf8>>},
        markdown:to_html(<<"[\n~\na\n-\n\n"/utf8>>),
        "1: label, blank lines, and code"
    ),
    ok.

-spec test_fuzz_case_2(Config) -> markdown_test:testcase() when
    Config :: ct_suite:ct_config().
test_fuzz_case_2(_Config) ->
    %% The first link is stopped by the `+` (so it’s `a@b.c`), but the next link overlaps it (`b.c+d@e.f`).
    ?assertMatch(
        {ok, <<"<p><a href=\"mailto:a@b.c\">a@b.c</a><a href=\"mailto:+d@e.f\">+d@e.f</a></p>"/utf8>>},
        markdown:to_html_with_options(<<"a@b.c+d@e.f"/utf8>>, markdown_options:gfm(#{})),
        "2: gfm: email autolink literals running into each other"
    ),
    ok.

-spec test_fuzz_case_3(Config) -> markdown_test:testcase() when
    Config :: ct_suite:ct_config().
test_fuzz_case_3(_Config) ->
    ?assertMatch(
        {ok, <<"<pre><code>x\n</code></pre>\n<ul>\n<li></li>\n</ul>"/utf8>>},
        markdown:to_html(<<"    x\n*    "/utf8>>),
        "3-a: containers should not pierce into indented code"
    ),
    ok.

-spec test_fuzz_case_4(Config) -> markdown_test:testcase() when
    Config :: ct_suite:ct_config().
test_fuzz_case_4(_Config) ->
    ?assertMatch(
        {ok, <<"<pre><code>a\n</code></pre>\n<ul>\n<li>\n<pre><code>b\n</code></pre>\n</li>\n</ul>"/utf8>>},
        markdown:to_html(<<"    a\n*     b"/utf8>>),
        "3-b: containers should not pierce into indented code"
    ),
    ok.

-spec test_fuzz_case_5(Config) -> markdown_test:testcase() when
    Config :: ct_suite:ct_config().
test_fuzz_case_5(_Config) ->
    ?assertMatch(
        {ok, <<"<p>a *</p>"/utf8>>},
        markdown:to_html(<<"a * "/utf8>>),
        "4-a: trailing whitespace and broken data"
    ),
    ok.

-spec test_fuzz_case_6(Config) -> markdown_test:testcase() when
    Config :: ct_suite:ct_config().
test_fuzz_case_6(_Config) ->
    ?assertMatch(
        {ok, <<"<p>_</p>"/utf8>>},
        markdown:to_html(<<"_  "/utf8>>),
        "4-b: trailing whitespace and broken data (GH-13)"
    ),
    ok.

-spec test_fuzz_case_7(Config) -> markdown_test:testcase() when
    Config :: ct_suite:ct_config().
test_fuzz_case_7(_Config) ->
    ?assertMatch(
        {ok, <<"<p>a ~</p>"/utf8>>},
        markdown:to_html_with_options(<<"a ~ "/utf8>>, markdown_options:gfm(#{})),
        "4-c: trailing whitespace and broken data (GH-14)"
    ),
    ok.

-spec test_fuzz_case_8(Config) -> markdown_test:testcase() when
    Config :: ct_suite:ct_config().
test_fuzz_case_8(_Config) ->
    ?assertEqual(
        {ok,
            markdown_mdast_node:root(#markdown_mdast_root{
                children = ?'vec!'([
                    markdown_mdast_node:list(#markdown_mdast_list{
                        children = ?'vec!'([
                            markdown_mdast_node:list_item(#markdown_mdast_list_item{
                                children = ?'vec!'([
                                    markdown_mdast_node:paragraph(#markdown_mdast_paragraph{
                                        children = ?'vec!'([
                                            markdown_mdast_node:text(#markdown_mdast_text{
                                                value = <<"ok">>,
                                                position = {some, markdown_unist_position:new(1, 12, 11, 1, 14, 13)}
                                            })
                                        ]),
                                        position = {some, markdown_unist_position:new(1, 12, 11, 1, 14, 13)}
                                    })
                                ]),
                                position = {some, markdown_unist_position:new(1, 1, 0, 1, 14, 13)},
                                spread = false,
                                checked = none
                            })
                        ]),
                        position = {some, markdown_unist_position:new(1, 1, 0, 1, 14, 13)},
                        ordered = true,
                        start = {some, 123456789},
                        spread = false
                    })
                ]),
                position = {some, markdown_unist_position:new(1, 1, 0, 1, 14, 13)}
            })},
        markdown:to_mdast(<<"123456789. ok"/utf8>>, markdown_parse_options:new(#{})),
        "5: lists should support high start numbers (GH-17)"
    ),
    ok.

-spec test_fuzz_case_9(Config) -> markdown_test:testcase() when
    Config :: ct_suite:ct_config().
test_fuzz_case_9(_Config) ->
    ?assertMatch(
        {ok, <<"<blockquote>\n<pre><code>\n</code></pre>\n</blockquote>"/utf8>>},
        markdown:to_html(<<"> ```\n"/utf8>>),
        "6-a: container close after unclosed fenced code, with eol (block quote, GH-16)"
    ),
    ok.

-spec test_fuzz_case_10(Config) -> markdown_test:testcase() when
    Config :: ct_suite:ct_config().
test_fuzz_case_10(_Config) ->
    ?assertMatch(
        {ok, <<"<ul>\n<li>\n<pre><code>\n</code></pre>\n</li>\n</ul>"/utf8>>},
        markdown:to_html(<<"- ```\n"/utf8>>),
        "6-b: container close after unclosed fenced code, with eol (list, GH-16)"
    ),
    ok.

-spec test_fuzz_case_11(Config) -> markdown_test:testcase() when
    Config :: ct_suite:ct_config().
test_fuzz_case_11(_Config) ->
    ?assertMatch(
        {ok, <<"<blockquote>\n<p>x</p>\n</blockquote>\n<p>``</p>"/utf8>>},
        markdown:to_html_with_options(<<"> x\n``"/utf8>>, markdown_options:gfm(#{})),
        "7: lazy container lines almost starting fenced code (GH-19)"
    ),
    ok.

-spec test_fuzz_case_12(Config) -> markdown_test:testcase() when
    Config :: ct_suite:ct_config().
test_fuzz_case_12(_Config) ->
    ?assertMatch(
        {ok, <<"<p>a\t<a href=\"mailto:b@c.d\">b@c.d</a></p>"/utf8>>},
        markdown:to_html_with_options(<<"a\tb@c.d"/utf8>>, markdown_options:gfm(#{})),
        "8-a: autolink literals after tabs (GH-18)"
    ),
    ok.

-spec test_fuzz_case_13(Config) -> markdown_test:testcase() when
    Config :: ct_suite:ct_config().
test_fuzz_case_13(_Config) ->
    ?assertMatch(
        {ok, <<"<p>aa\t<a href=\"mailto:b@c.d\">b@c.d</a></p>"/utf8>>},
        markdown:to_html_with_options(<<"aa\tb@c.d"/utf8>>, markdown_options:gfm(#{})),
        "8-b: autolink literals after tabs (GH-18)"
    ),
    ok.

-spec test_fuzz_case_14(Config) -> markdown_test:testcase() when
    Config :: ct_suite:ct_config().
test_fuzz_case_14(_Config) ->
    ?assertMatch(
        {ok, <<"<p>aaa\t<a href=\"mailto:b@c.d\">b@c.d</a></p>"/utf8>>},
        markdown:to_html_with_options(<<"aaa\tb@c.d"/utf8>>, markdown_options:gfm(#{})),
        "8-c: autolink literals after tabs (GH-18)"
    ),
    ok.

-spec test_fuzz_case_15(Config) -> markdown_test:testcase() when
    Config :: ct_suite:ct_config().
test_fuzz_case_15(_Config) ->
    ?assertMatch(
        {ok, <<"<p>aaaa\t<a href=\"mailto:b@c.d\">b@c.d</a></p>"/utf8>>},
        markdown:to_html_with_options(<<"aaaa\tb@c.d"/utf8>>, markdown_options:gfm(#{})),
        "8-d: autolink literals after tabs (GH-18)"
    ),
    ok.

-spec test_fuzz_case_16(Config) -> markdown_test:testcase() when
    Config :: ct_suite:ct_config().
test_fuzz_case_16(_Config) ->
    ?assertMatch(
        {ok, <<"""
        <table>
        <thead>
        <tr>
        <th>a</th>
        </tr>
        </thead>
        <tbody>
        <tr>
        <td><a href="http://www.a">www.a</a></td>
        </tr>
        </tbody>
        </table>
        """/utf8>>},
        markdown:to_html_with_options(<<"| a |\n| - |\n| www.a|"/utf8>>, markdown_options:gfm(#{})),
        "9: autolink literals that end in table cell delimiter (GH-20)"
    ),
    ok.

-spec test_fuzz_case_17(Config) -> markdown_test:testcase() when
    Config :: ct_suite:ct_config().
test_fuzz_case_17(_Config) ->
    ?assertMatch(
        {ok, <<"<p><a href=\"\">*</a> <a href=\"\">*</a></p>"/utf8>>},
        markdown:to_html_with_options(<<"[*]() [*]()"/utf8>>, markdown_options:gfm(#{})),
        "10: attention in different links (GH-21)"
    ),
    ok.

-spec test_fuzz_case_18(Config) -> markdown_test:testcase() when
    Config :: ct_suite:ct_config().
test_fuzz_case_18(_Config) ->
    ?assertEqual(
        {ok,
            markdown_mdast_node:root(#markdown_mdast_root{
                children = ?'vec!'([
                    markdown_mdast_node:list(#markdown_mdast_list{
                        children = ?'vec!'([
                            markdown_mdast_node:list_item(#markdown_mdast_list_item{
                                children = ?'vec!'([
                                    markdown_mdast_node:paragraph(#markdown_mdast_paragraph{
                                        children = ?'vec!'([
                                            markdown_mdast_node:text(#markdown_mdast_text{
                                                value = <<"[ ]\na">>,
                                                position = {some, markdown_unist_position:new(1, 3, 2, 2, 2, 7)}
                                            })
                                        ]),
                                        position = {some, markdown_unist_position:new(1, 3, 2, 2, 2, 7)}
                                    })
                                ]),
                                position = {some, markdown_unist_position:new(1, 1, 0, 2, 2, 7)},
                                spread = false,
                                checked = none
                            })
                        ]),
                        position = {some, markdown_unist_position:new(1, 1, 0, 2, 2, 7)},
                        ordered = false,
                        start = none,
                        spread = false
                    })
                ]),
                position = {some, markdown_unist_position:new(1, 1, 0, 2, 2, 7)}
            })},
        markdown:to_mdast(<<"* [ ]\na"/utf8>>, markdown_parse_options:new(#{})),
        "11: gfm task list items followed by eols (GH-24)"
    ),
    ok.

-spec test_fuzz_case_19(Config) -> markdown_test:testcase() when
    Config :: ct_suite:ct_config().
test_fuzz_case_19(_Config) ->
    ?assertMatch(
        {ok, <<"<p>&lt;</p>"/utf8>>},
        markdown:to_html_with_options(<<"<"/utf8>>, markdown_options:mdx(#{})),
        "12: mdx: handle invalid mdx without panic (GH-26)"
    ),
    ok.
