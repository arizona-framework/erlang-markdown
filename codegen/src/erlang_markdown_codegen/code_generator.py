...
class SignedSourceErl(Extension):
    PREFIX: str = "".join(["%" * 3, " %"])
    HEADER: str = "".join(
        [PREFIX, " DO NOT EDIT: this file was generated by 'just codegen'\n", PREFIX, " @", "g", "enerated ", TOKEN, "\n"]
    )
    ...
    def __post_init__(self) -> None:
        ...
        for template_name in glob.iglob("**", root_dir=self.templates_path, recursive=True):
            if os.path.isfile(os.path.join(self.templates_path, template_name)):
                ...
                if template_ext == ".j2":
                    self.targets.append(StageTarget(stage=self, template_name=template_name, output_name=output_name))
...
class CodeGenerator:
    ...
    def __post_init__(self) -> None:
        with open(self.config_file, "r", encoding="utf-8") as f:
            ...
            self.ctx = Context(self.output_path, config)
    ...
    def render(self) -> None:
        ...
        print("Stage0 Render")
        self.stage0.render()
        # self._analyze_c_src_drv_atoms()
        self._analyze_c_src_nif_atoms()
        print("Stage1 Render")
        self.stage1.render()

    def _analyze_c_src_drv_atoms(self) -> None:
        c_src_drv_root: str = os.path.join(self.output_path, "apps", "chaos", "c_src", "drv")
        for source_name in glob.iglob("**", root_dir=c_src_drv_root, recursive=True):
            source_file: str = os.path.join(c_src_drv_root, source_name)
                ...
                if actual_ext in (".c", ".h", ".c.h"):
                    with open(source_file, "r", encoding="utf-8") as f:
                        ...
                        for atom_name in re.finditer(r"\bATOM\((.*?)\)", contents):
                            self.ctx.drv.make_atom(atom_name.group(1))

    def _analyze_c_src_nif_atoms(self) -> None:
        c_src_nif_root: str = os.path.join(self.output_path, "apps", "chaos", "c_src", "nif")
